...
`stan/math.hpp` は主役と呼ぶべきもので、確率分布からのサンプリングなど全体を通して使用します。そのため、 `stan`、`math` および `Eigen` に対して Using 宣言をしてしまっていますが、衝突が起きそうな関数に対して敢えて名前空間で修飾している箇所もあります。とくに線形代数ライブラリである Eigen は Stan Math Library が依存しているライブラリで、 Stan Math Library 内の関数のうちいくつかでは、その入出力が Eigen で定義されるベクトル・行列の型になっています。

### デモデータ生成関数

早速ですが、デモデータを生成するための関数 `generate_data` を定義します。今回はポアソン混合モデルなので、入力として以下のものを考えます。
...
当然ここで、 `pi` は総和が 1 になるようなものを想定しています。まずは、全体をお見せします。
...
1. 乱数生成器の設定
2. 変数の宣言
3. サンプリングしつつ `data.csv` にその結果を書き出し

2 では、サンプリングしたい潜在変数 $s_{n}$ とデータ $X$ をそれぞれ `s` および `X` として宣言しています。

3 の `for` 文の中では、$x_{n}$ をサンプリングすることを考え、まずは混合比率 `pi` をパラメータにもつカテゴリ分布から潜在変数 `s` をサンプリングします。このとき、書籍では潜在変数が 1 of K (one-hot) 表現になっていますが、`s` は単純に $1 \ldots K$ の値をとります。
...
大丈夫そうですね。クラスター数を増やしたり、サンプルサイズを増やしたりすることもできます。

というわけで、今回はデモデータを生成する関数を実装しました。以降の記事ではこの関数を使って生成されたデモデータに対して推論をします。お疲れさまでした。
